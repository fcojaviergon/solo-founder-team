#!/usr/bin/env python3
"""
Generate PDP quotation Excel spreadsheet.
Usage: python generate_pdp_xlsx.py <output_path> <json_data_path>
...
JSON structure:
{
  "tasks": [
    {
      "id": "M1-T01",
      "modulo": "Setup & Infrastructure",
      "tarea": "Configure Docker compose",
      "tipo": "Infra",
      "complejidad": "Low",
      "hh": 8,
      "perfil": "Senior",
...
COMP_COLORS = {
    ...
    'Very High': PatternFill('solid', fgColor='F8D7DA'),
}
RISK_COLORS = {
    'Low': PatternFill('solid', fgColor='D4EDDA'),
    'Medium': PatternFill('solid', fgColor='FFF3CD'),
...
def wbs_sheet(wb, tasks):
    ...
    headers = ['ID','Module','Task','Type','Complexity','MH','Profile','Dependencies','Risk','Phase','Notes']
    widths = [10,20,40,12,12,10,16,14,10,12,30]
    ws.merge_cells('A1:K1')
    ws['A1'].value = "WBS — Work Breakdown Structure"
    ...
    for t in tasks:
        ...
        row += 1
    lr = row - 1
    ws.cell(row=row+1, column=5, value="TOTAL MH:").font = TOTAL_FONT
...
def summary_module(wb, tasks):
    ws = wb.create_sheet("Summary by Module")
    ws.merge_cells('A1:D1')
    ws['A1'].value = "Summary by Module"
    ws['A1'].font = Font(bold=True, color='1B2A4A', size=14, name='Arial')
    for c,(h,w) in enumerate(zip(['Module','Tasks','MH','%'],[30,12,12,12]),1):
        ws.cell(row=3, column=c, value=h)
        ws.column_dimensions[get_column_letter(c)].width = w
    hdr(ws, 3, 4)
    mods = {}
...
def summary_profile(wb, tasks):
    ws = wb.create_sheet("Summary by Profile")
    ws.merge_cells('A1:D1')
    ws['A1'].value = "Summary by Profile"
    ws['A1'].font = Font(bold=True, color='1B2A4A', size=14, name='Arial')
    for c,(h,w) in enumerate(zip(['Profile','Tasks','MH','%'],[25,12,12,12]),1):
        ws.cell(row=3, column=c, value=h)
        ws.column_dimensions[get_column_letter(c)].width = w
    hdr(ws, 3, 4)
    profs = {}
...
def cost_calc(wb, tasks):
    ...
    ws['A1'].value = "Cost Calculator"
    ws['A1'].font = Font(bold=True, color='1B2A4A', size=14, name='Arial')
    ws.merge_cells('A2:F2')
    ws['A2'].value = "Enter rates in columns C and E (blue = editable)"
    ws['A2'].font = Font(italic=True, color='666666', size=10, name='Arial')
    for c,(h,w) in enumerate(zip(
        ['Profile','MH','Rate/Hr USD','Cost USD','Rate/Hr Local','Cost Local'],
        [25,12,18,18,18,18]),1):
        ws.cell(row=3, column=c, value=h)
        ws.column_dimensions[get_column_letter(c)].width = w
    hdr(ws, 3, 6)
    ...
    sr = row
    row += 2
    ws.cell(row=row, column=1, value="CONTINGENCY").font = SUB_FONT
    row += 1
    ...
    br = row
    row += 1
    ws.cell(row=row, column=1, value="MH Buffer").font = DATA_FONT
    ws.cell(row=row, column=2, value=f'=B{sr}*B{br}').font = DATA_FONT
    bhr = row
    row += 1
    ws.cell(row=row, column=1, value="TOTAL WITH BUFFER").font = TOTAL_FONT
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python generate_pdp_xlsx.py <output.xlsx> <data.json>")
        sys.exit(1)
    ...
    cost_calc(wb, tasks)
    wb.save(sys.argv[1])
    print(f"Excel generated: {sys.argv[1]}")
